...
def shared_syllabi_queryset(user):
    ...
    if user.role in ["dean", "umu", "admin"] or user.is_superuser or getattr(user, "is_admin_like", False):
        return qs
...
def can_view_syllabus(user, syllabus: Syllabus) -> bool:
    if user == syllabus.creator:
        return True
    if user.role in ["dean", "umu", "program_leader", "admin"] or user.is_superuser or getattr(user, "is_admin_like", False):
        return True
